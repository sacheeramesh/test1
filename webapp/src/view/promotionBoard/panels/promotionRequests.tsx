...
import {
  ...
  getFLApprovedPromotionRequests,
  rejectPromotionRequest,
  approvePromotionRequestList,
  rejectPromotionRequestList,
} from "@slices/promotionBoardSlice/activeRequest";
...
export default function FLApprovedList() {
  ...
  const fl_approvedList = useAppSelector(
    (state: RootState) => state.fL_approvedRequest
  );
            ...
            <IconButton
              ...
              onClick={() => {
                dialogContext.showConfirmation(
                  ...
                  () => {
                    dispatch(approvePromotionRequest(data.id));
                  },
                  "approve"
                );
              }}
  ...
  const multipleApprove = (ids: number[]) => {
    dialogContext.showConfirmation(
      ...
      () => {
        dispatch(approvePromotionRequestList(ids));
      },
      "Approve"
    );
  };
        ...
        {fl_approvedList.state === "success" &&
          fl_approvedList.requests?.length === 0 && (
            <StateWithImage
              ...
              message="There are no active promotion requests"
            />
          )}

        {fl_approvedList.state === "failed" && (
          <StateWithImage
            imageUrl="/warning.svg"
            message="Something went wrong, while fetching active promotion requests"
          />
        )}
      </Box>
      {/* Edit Dialog */}
